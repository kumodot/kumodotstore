    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-text-primary">
            Admin — <span className="text-accent">Color Manager</span>
          </h1>
        ...
        <button
          onClick={() => {
            if (confirm("Reset all colors to factory defaults?")) {
              resetToDefaults();
            }
          }}
      ...
      <div className="mt-6 p-4 bg-surface-card border border-border rounded-xl">
        <p className="text-xs text-text-muted">
          <span className="text-text-secondary font-medium">Note:</span> Changes are stored in
          your browser's localStorage. To share your color config with other devices, export it
          as JSON (coming soon). Color IDs are used in order codes — do not change the ID of an
          existing color or old codes will break.
        </p>
      </div>
    </div>
  );
